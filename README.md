...
- [PickySteve](https://github.com/KernelLord/pickysteve) - Local-first skill router and context picker for coding agents: hybrid BM25 + embedding retrieval with cross-encoder rerank and an LLM judge selects the right skill, then assembles a minimal nonce-boundaried context bundle. Built-in fail-closed prompt-injection gate scans both the request and every retrieved document. Runs on local Ollama by default; MCP stdio server and OpenAI-compatible proxy wire into 18 coding agents including Codex CLI.
