...
- [DevIntern](https://github.com/getdevintern/devintern) - Tool that picks up tickets from Jira, Linear, Trello, Asana, Azure DevOps, GitHub Issues, or markdown files and drives Codex non-interactively to turn them into self-reviewed pull requests (also works with Claude Code, Cursor, OpenCode, and Gemini CLI). A feasibility gate flags vague tickets back to the tracker with questions; optional unattended mode schedules ticket pickup and turns PR review comments into commits. Runs on your machines with your own OpenAI keys or subscription. FSL-1.1, free for interactive use.
