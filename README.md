...
- [Nightly Closeout Skill](https://github.com/Xinnygh/nightly-closeout-skill) by [Xinnygh](https://github.com/Xinnygh) - MIT Codex skill that turns leftover end-of-day token/model capacity into safe overnight Codex/Claude work loops. It triages your currently active projects, scores high-value low-risk overnight tasks, generates separate Codex (strategy/review) and Claude (execution/research) loop prompts, and uses Codex Goals to hold a durable objective across long runs. Conservative by default: it will not send, publish, spend, sign, or delete unattended.
